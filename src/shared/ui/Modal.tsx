...
export function Modal({ onClose, title, message, children, ...rest }: ModalProps) {
  return (
    ...
    >
      ...
      >
        <Pressable
          className="w-[300px] rounded-xl bg-white px-5 py-7 flex-col items-center gap-4"
          onPress={e => e.stopPropagation()}
        >
          {title && (
            <Text className="text-center text-base font-bold text-text-black tracking-tight leading-[22.4px]">
              {title}
            </Text>
          )}
          {message && (
          ...
          {rest.visible && children && (
            <View className="w-[260px] self-center">
              {children}
            </View>
          )}
        </Pressable>
